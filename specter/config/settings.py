"""Specter configuration management — load, save, and access settings."""
...
CONFIG_DIR = Path(os.environ.get("SPECTER_CONFIG_DIR", str(Path.home() / ".specter")))
...
def openai_default_headers() -> dict[str, str]:
    return {"User-Agent": os.environ.get("SPECTER_LLM_USER_AGENT", DEFAULT_OPENAI_USER_AGENT)}
...
def load_config() -> SpecterConfig:
    ...
    config = SpecterConfig(mcp=MCPServersConfig(servers=servers))
...
def _merge_config(base: SpecterConfig, raw: dict[str, Any]) -> SpecterConfig:
...
def _overlay_env(config: SpecterConfig) -> SpecterConfig:
    """Overlay environment variables onto config.

    Supported env vars (prefix SPECTER_):
    ...
    """
    # ── LLM ──────────────────────────────────────────────────────────
    if v := os.environ.get("SPECTER_LLM_API_KEY"):
        config.llm.api_key = v
    if v := os.environ.get("SPECTER_LLM_BASE_URL"):
        config.llm.base_url = v
    if v := os.environ.get("SPECTER_LLM_MODEL"):
        config.llm.model = v
    if v := os.environ.get("SPECTER_LLM_PROVIDER"):
        config.llm.provider = v
    if v := os.environ.get("SPECTER_LLM_MAX_TOKENS"):
        with suppress(ValueError):
            config.llm.max_tokens = int(v)
    if v := os.environ.get("SPECTER_LLM_MAX_CONTEXT_TOKENS"):
        with suppress(ValueError):
            config.llm.max_context_tokens = int(v)
    if v := os.environ.get("SPECTER_LLM_TEMPERATURE"):
        with suppress(ValueError):
            config.llm.temperature = float(v)

    # ── Session ──────────────────────────────────────────────────────
    if v := os.environ.get("SPECTER_SESSION_OUTPUT_DIR"):
        config.session.output_dir = Path(v)
    if v := os.environ.get("SPECTER_SESSION_AUTO_SAVE"):
        config.session.auto_save = v.lower() in ("1", "true", "yes", "on")
    if v := os.environ.get("SPECTER_SESSION_REPORT_FORMAT"):
        config.session.report_format = v
    if v := os.environ.get("SPECTER_SESSION_MAX_ROUNDS"):
        with suppress(ValueError):
            config.session.max_rounds = int(v)
    if v := os.environ.get("SPECTER_SESSION_SHOW_THINKING"):
        config.session.show_thinking = v.lower() in ("1", "true", "yes", "on")
    if v := os.environ.get("SPECTER_SESSION_STALE_ROUNDS_THRESHOLD"):
        with suppress(ValueError):
    ...
    if v := os.environ.get("SPECTER_SESSION_REFLEXION_ENABLED"):
        config.session.reflexion_enabled = v.lower() in _truthy
    if v := os.environ.get("SPECTER_SESSION_REFLEXION_MAX_SAME_VULN_FAILS"):
        with suppress(ValueError):
            config.session.reflexion_max_same_vuln_fails = int(v)
    if v := os.environ.get("SPECTER_SESSION_REFLEXION_MAX_TOTAL_NO_PROGRESS"):
        with suppress(ValueError):
            config.session.reflexion_max_total_no_progress = int(v)
    if v := os.environ.get("SPECTER_SESSION_ESCALATION_MAX_LEVEL"):
        with suppress(ValueError):
            config.session.escalation_max_level = int(v)
    if v := os.environ.get("SPECTER_SESSION_PLUGIN_RUNTIME_ENABLED"):
        config.session.plugin_runtime_enabled = v.lower() in _truthy
    if v := os.environ.get("SPECTER_SESSION_PLUGIN_DEFAULT_TIMEOUT"):
        with suppress(ValueError):
            config.session.plugin_default_timeout = int(v)
    if v := os.environ.get("SPECTER_SESSION_PLUGIN_MAX_REQUESTS_PER_TARGET"):
        with suppress(ValueError):
            config.session.plugin_max_requests_per_target = int(v)
    if v := os.environ.get("SPECTER_SESSION_EVIDENCE_MIN_REPORT_LEVEL"):
        config.session.evidence_min_report_level = v

    # ── Safety ───────────────────────────────────────────────────────
    if v := os.environ.get("SPECTER_SAFETY_PYTHON_EXECUTE_ENABLED"):
        config.safety.enable_python_execute = v.lower() in ("1", "true", "yes", "on")
    if v := os.environ.get("SPECTER_SAFETY_PYTHON_EXECUTE_RESTRICTED"):
        config.safety.python_execute_restricted = v.lower() in ("1", "true", "yes", "on")
    if v := os.environ.get("SPECTER_SAFETY_PYTHON_EXECUTE_MODE"):
        config.safety.python_execute_mode = v
    if v := os.environ.get("SPECTER_SAFETY_PYTHON_EXECUTE_MAX_LINES"):
        with suppress(ValueError):
            config.safety.python_execute_max_lines = int(v)
    if v := os.environ.get("SPECTER_SAFETY_PYTHON_EXECUTE_SHOW_WARNING"):
        config.safety.python_execute_show_warning = v.lower() in ("1", "true", "yes", "on")
    if v := os.environ.get("SPECTER_SAFETY_PYTHON_EXECUTE_MAX_OUTPUT_CHARS"):
        with suppress(ValueError):
            config.safety.python_execute_max_output_chars = int(v)
    if v := os.environ.get("SPECTER_SAFETY_PYTHON_EXECUTE_AUDIT_ENABLED"):
    ...
    # (SPECTER_RECON_FOFA_KEY); short form wins if both are set.
    for field, names in {
        "fofa_email": ("FOFA_EMAIL", "SPECTER_RECON_FOFA_EMAIL"),
        "fofa_key": ("FOFA_KEY", "SPECTER_RECON_FOFA_KEY"),
        "hunter_key": ("HUNTER_KEY", "SPECTER_RECON_HUNTER_KEY"),
        "quake_key": ("QUAKE_KEY", "SPECTER_RECON_QUAKE_KEY"),
        "zoomeye_key": ("ZOOMEYE_KEY", "SPECTER_RECON_ZOOMEYE_KEY"),
        "shodan_key": ("SHODAN_KEY", "SPECTER_RECON_SHODAN_KEY"),
        "zerozone_key": ("ZEROZONE_KEY", "SPECTER_RECON_ZEROZONE_KEY"),
    }.items():
...
def apply_provider_preset(config: SpecterConfig, provider_name: str) -> SpecterConfig:
