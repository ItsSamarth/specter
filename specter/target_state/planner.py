...
def build_resume_plan(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    if pending_sorted:
        next_actions = [
            "Re-test high-confidence pending vulnerabilities first",
        ...
        if blocked_targets:
            next_actions.append("Skip confirmed-unreachable targets and focus on verifying still-accessible entry points")
        if low_value_rounds >= 3:
            next_actions.append("Many consecutive low-value rounds; switch the parameter surface or find a new entry point first")
        if recon_priority_assets:
            next_actions.append(f"Return to high-value recon assets first: {recon_priority_assets[0]}")
        return {
    ...
    if verified_sorted:
        next_actions = [
            "Prioritize extending the exploitation chain around verified vulnerabilities",
            "Avoid regressing to low-value basic recon",
        ]
        if violation_events:
            next_actions.append("Before extending exploitation, confirm it will not trigger existing constraint policies")
        if current_attack_path:
            next_actions.append(f"Do not stay stuck on the old path {current_attack_path}; prioritize new follow-up exploitation")
        if recon_priority_assets:
            next_actions.append(f"Extend exploitation using high-value recon assets: {recon_priority_assets[0]}")
        return {
    ...
    if incomplete:
        next_actions = ["Fill recon gaps before moving to vulnerability verification"]
        if violation_events:
            next_actions.append("Prefer recon actions not blocked by constraints")
        if blocked_targets:
            next_actions.append("Ignore unreachable sub-targets and complete recon dimensions for still-accessible assets first")
        if recon_priority_assets:
            next_actions.append(f"Continue with these high-value recon assets first: {recon_priority_assets[0]}")
        return {
    ...
    if recon_priority_assets:
        next_actions.append(f"Test these high-value recon assets first: {recon_priority_assets[0]}")
    return {
        "strategy": "continue_scan",
        "reason": _build_reason(
            "No verified vulnerabilities yet; keep advancing scanning from candidate attack surfaces",
            blocked_targets=blocked_targets,
            low_value_rounds=low_value_rounds,
        ),
        "recommended_phase": PentestPhase.VULN_DISCOVERY.value,
...
def _build_reason(base: str, *, blocked_targets: list[str], low_value_rounds: int) -> str:
    suffix: list[str] = []
    if blocked_targets:
        suffix.append(f"{len(blocked_targets)} unreachable target(s) already exist")
    if low_value_rounds >= 3:
