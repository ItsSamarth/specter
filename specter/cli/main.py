"""Specter CLI main entry point with REPL and sub-commands."""
...
from specter.agent.constraint_policy import validate_action_constraints
...
from specter.config.settings import (
...
from specter.i18n import _
from specter.orchestrator import run_agent_task
from specter.repl_runner import run_repl_call
...
# Place after the top-of-file imports, before the app definition
...
app = typer.Typer(
    name="specter",
    help="Specter - AI-powered penetration testing CLI (run 'specter tui' for the TUI workbench)",
...
BANNER_SUBTITLE = f"Specter v{__version__} - AI-powered penetration testing CLI"
    ...
    def on_event(kind: str, payload: dict) -> None:
        if kind == "reason":
            ...
            if complete_flag is not None and complete_flag is not False:
                # Leave the completion announcement to the post-verification completed / complete_rejected events,
                # to avoid the mismatch of "announce goal met, then get rejected"
                pass
            ...
            target_console.print(
                f"[red]⚠ Hallucination block {payload['intent_id']}:[/red] the claimed flag has no real evidence; rejected"
            )
...
def _run_repl() -> None:
    ...
    while True:
        try:
            ...
            prompt_str = " | ".join(prompt_parts) if prompt_parts else "specter"

            # Read input
            user_input = console.input(f"specter {prompt_str}> ").strip()
            ...
            if cmd_lower in ("exit", "quit", "q"):
                ...
                from specter.agent.core import PersistentCycleResult
                ...
                def _on_persistent_step(round_num: int, cycle_num: int, result) -> None:
                    ...
                    # TerminalStreamSink already streams in real time; the callback does not print again
                    console.print()
            ...
            try:
                if is_auto_mode:
                    ...
                    # Default to the goal-driven solve engine; engine=rounds falls back to the old fixed-round loop
                    ...
                    async def _run_agent():
                        ...
                        async def after_result(result):
                            ...
                            if result:
                                ...
                                if result.phase:
                                    current_phase = result.phase
                                # Commented out: streaming output already shown live via TerminalStreamSink; no need to print again
                                # if result.output:
...
def _generate_report_for_target(
    ...
    *,
    ...
    from specter.report.generator import generate_report, generate_report_from_target_state
    from specter.target_state.store import load_target_state
...
def run(
    ...
    # [new] 2026-06-10 Nyaecho - TUI natural-language driver: allow a custom prompt via --prompt to override the auto-generated one
...
def solve(
    ...
    resolved_goal = goal or "find a flag / get a shell / confirm and verify high-value vulnerabilities"
    task_prompt = prompt or (
        f"Perform an authorized penetration test of {target}. This is an explicitly authorized, in-scope target. Goal: {resolved_goal}."
    )
...
def recon(
    ...
    async def _run():
        async def runner(agent, _config):
            sink = TerminalStreamSink(console, _config.session.show_thinking)
            # TerminalStreamSink already streams in real time; no repeated console.print
...
def report(
    ...
    if target_mode:
        from specter.report.generator import generate_report_from_target_state
        ...
        from specter.report.generator import generate_report_from_file
...
def config_provider(
    ...
    if list_all or name is None:
        ...
        for p in providers:
            ...
            console.print()
        console.print("[dim]Use specter config provider <name> to switch providers.[/]")
        return

    # Switch provider
    from specter.config.schema import PROVIDER_PRESETS, LLMProvider
    ...
    if not config.llm.api_key:
        console.print()
        console.print(
            "[yellow]Set an API key first: [bold]specter config set llm.api_key <your-key>[/][/]"
        )
...
def init() -> None:
    ...
    from specter.config.settings import ensure_dirs
...
@app.command()
def doctor() -> None:
    """Inspect the Specter runtime environment."""
    import shutil

    from specter.web.services.mcp_service import get_mcp_diagnostics

    console.print("[bold]Specter Environment Check[/]")
    console.print()
    ...
    console.print()
    if has_key:
        console.print("[green]Environment ready. Run [bold]specter[/] to start.[/]")
    else:
        console.print(
            "[yellow]Set an API key first: [bold]specter config set llm.api_key <key>[/][/]"
        )
...
def _parse_kv_options(pairs: Optional[list[str]]) -> dict[str, object]:
    """Parse --option key=value (repeatable) into a dict, parsing the value as JSON when possible."""
...
def plugins_list(
    ...
    from specter.plugins import registry
    ...
    table = Table(title="Specter Plugins", show_lines=False)
...
def plugins_run(
    ...
    from specter.plugins import PluginContext, PluginStage, create_builtin_runtime
...
def kb_update() -> None:
    ...
    from specter.kb.updater import seed_knowledge_base
...
def kb_status() -> None:
    ...
    from specter.kb.retriever import KnowledgeRetriever, RetrieverStatus
    from specter.kb.store import KnowledgeStore
    ...
    console.print(
        Panel(
            ...
            f"Semantic search: run [bold]pip install specter\\[kb][/] to enable ChromaDB",
            title="KB Status",
            border_style="cyan",
        )
    )
...
def _should_auto_pentest(user_input: str, current_target: Optional[str]) -> bool:
    ...
    - User mentions a target plus action keywords like "pentest" or "go at it"
    ...
    # But if it also has auto keywords, still go auto (e.g. "gather info and generate a report")
...
def _auto_save_recon_report(agent, user_input: str, config) -> None:
    ...
    try:
        ...
        from specter.report.generator import generate_report
...
def tui(
    ...
    if once and target:
        from specter.cli.tui import render_tui_home
...
def web(
    ...
    from specter.web.app import FASTAPI_AVAILABLE
    ...
    if not FASTAPI_AVAILABLE:
        err_console.print(
            "[!] FastAPI is missing. Install with [bold]pip install specter[web][/]."
        )
        raise typer.Exit(1)
    ...
    try:
        ...
        err_console.print(
            "[!] uvicorn is missing. Install with [bold]pip install specter[web][/]."
        )
        raise typer.Exit(1)

    from specter.web.app import create_app
