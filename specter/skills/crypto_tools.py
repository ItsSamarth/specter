"""Specter Crypto Toolkit — encoding/decoding, encryption/decryption utilities.
...
@_register("rot13_encode", "encode", "ROT13 encode (self-inverse; encoding equals decoding)", ["input"])
...
@_register(
    ...
    ["input"],
    {"key": "Key (16/24/32 bytes)", "iv": "Initialization vector (16 bytes; defaults to the same as the key)"},
)
def _aes_encrypt(input_str: str, key: str = "", iv: str = "", **_) -> dict:
    try:
        ...
        return {
            "success": False,
            "result": "",
            "error": "Requires pycryptodome: pip install pycryptodome",
        }
    except Exception as e:
        return {"success": False, "result": "", "error": f"AES encryption failed: {e}"}
...
def _aes_decrypt(input_str: str, key: str = "", iv: str = "", **_) -> dict:
    try:
        ...
        return {"success": False, "result": "", "error": f"AES decryption failed: {e}"}
...
def execute(operation: str, input_str: str, **kwargs) -> dict:
    ...
    try:
        ...
        return {"success": False, "result": "", "error": f"Error while executing {operation}: {e}"}
