"""FastAPI app entry for the Specter Web UI backend."""

from __future__ import annotations

from pathlib import Path

from specter.web.schemas import ConfigUpdateRequest, ReportGenerateRequest, TaskCreateRequest
from specter.web.services.config_service import get_public_config, update_public_config
...
from specter.web.services.report_service import (
    generate_target_report,
    list_reports,
    read_report_content,
    resolve_report_path,
)
from specter.web.services.target_service import (
    clear_target,
    get_diff,
    get_preview,
    get_snapshots,
...
from specter.web.services.task_service import start_task
...
def create_app():
    ...
    if not FASTAPI_AVAILABLE:
        raise RuntimeError(
            "FastAPI is not installed. Install the web extra first: pip install specter[web]"
        )

    app = FastAPI(title="Specter Web UI", version="0.3.2")

    @app.get("/api/health")
    async def health():
        return {"status": "ok", "service": "specter-web"}
