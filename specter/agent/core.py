"""Specter Agent Core — the main AI agent loop with tool calling."""

from __future__ import annotations

from typing import Any, Callable, Optional

from specter.agent.anti_loop import (
    detect_attack_path,
...
from specter.agent.builtin_tools import (
    BLOCKED_PATTERNS,
    RESERVED_IP_RANGES,
    build_openai_tools,
    execute_mcp_tool,
...
from specter.agent.context import ContextManager, PentestPhase, SessionState, TaskConstraints
from specter.agent.ctf_mode import detect_flag_claim
from specter.agent.finding_parser import FindingParser
from specter.agent.input_analysis import (
    detect_phase,
    detect_target,
...
from specter.agent.kb_context import build_kb_context
from specter.agent.llm_client import StreamSink, call_llm
from specter.agent.loop_controller import auto_pentest as run_auto_pentest
from specter.agent.loop_controller import persistent_pentest as run_persistent_pentest
from specter.agent.prompt_context import build_round_context, generate_attack_summary
from specter.agent.recon_tracker import update_recon_dimension_completion
from specter.agent.runtime_state import AgentResult, PersistentCycleResult, RuntimeState
from specter.agent.skill_context import get_active_skill_context
from specter.agent.system_prompt import build_dynamic_system_prompt
from specter.agent.tool_call_manager import safe_parse_tool_args
from specter.config.schema import SpecterConfig
from specter.config.settings import make_openai_client
from specter.target_state.store import save_target_state
...
class AgentCore:
    ...
    def __init__(self, config: SpecterConfig, mcp_manager: Any = None) -> None:
        self.config = config
    ...
    def _restore_reflexion_history(self) -> None:
        ...
        if not snapshot or reflexion is None:
            return
        try:
            from specter.agent.reflexion import ReflexionState
    ...
    def _extract_response(message: Any) -> str:
        ...
        from specter.agent.llm_client import extract_response
