"""Specter Config Module Tests — schema.py + settings.py"""
...
class TestLLMConfig:
    ...
    def test_default_values(self):
        from specter.config.schema import LLMConfig
...
class TestMCPServerConfig:
    ...
    def test_default_values(self):
        from specter.config.schema import MCPServerConfig, MCPTransportConfig
...
class TestSessionConfig:
    ...
    def test_runtime_integration_default_values(self):
        from specter.config.schema import SessionConfig
...
class TestSpecterConfig:
    ...
    def test_mcp_builtin_servers(self):
        from specter.config.schema import BUILTIN_MCP_SERVERS, SpecterConfig
    ...
    def test_builtin_mcp_server_count(self):
        from specter.config.schema import BUILTIN_MCP_SERVERS
    ...
    def test_provider_presets(self):
        from specter.config.schema import PROVIDER_PRESETS
    ...
    def test_llm_provider_enum(self):
        from specter.config.schema import LLMProvider
...
class TestSettingsLoad:
    ...
    def test_load_config_returns_config(self):
        ...
        config = load_config()
        assert isinstance(config, SpecterConfig)
    ...
    def test_save_config(self):
        from specter.config.schema import SpecterConfig
        from specter.config.settings import save_config
    ...
    def test_set_config_value(self):
        from specter.config.settings import set_config_value
    ...
    def test_set_config_mcp_server_field(self):
        from specter.config.settings import load_config, set_config_value
    ...
    def test_apply_provider_preset(self):
        from specter.config.schema import SpecterConfig
        from specter.config.settings import apply_provider_preset
    ...
    def test_list_providers(self):
        from specter.config.settings import list_providers
    ...
    def test_env_var_override(self, monkeypatch):
        ...
        monkeypatch.setenv("SPECTER_LLM_API_KEY", "env-test-key")
        monkeypatch.setenv("SPECTER_LLM_MODEL", "env-test-model")
    ...
    def test_openai_default_headers_allow_user_agent_override(self, monkeypatch):
        from specter.config.settings import openai_default_headers

        assert openai_default_headers()["User-Agent"] == "Mozilla/5.0"

        monkeypatch.setenv("SPECTER_LLM_USER_AGENT", "test-agent")
    ...
    def test_env_var_override_new_session_fields(self, monkeypatch):
        ...
        monkeypatch.setenv("SPECTER_SESSION_REASONING_STATE_ENABLED", "false")
        monkeypatch.setenv("SPECTER_SESSION_REFLEXION_MAX_SAME_VULN_FAILS", "5")
        monkeypatch.setenv("SPECTER_SESSION_ESCALATION_MAX_LEVEL", "2")
        monkeypatch.setenv("SPECTER_SESSION_PLUGIN_RUNTIME_ENABLED", "false")
        monkeypatch.setenv("SPECTER_SESSION_PLUGIN_MAX_REQUESTS_PER_TARGET", "7")
        monkeypatch.setenv("SPECTER_SESSION_EVIDENCE_MIN_REPORT_LEVEL", "L2")
