from specter.agent.reflexion import (
    FailureCategory,
    ReflexionEngine,
    classify_failure,
)
...
def test_escalation_hints_are_level_specific_and_capped():
    ...
    assert engine.get_escalation_hints() == ["Try the raw payload first (no encoding)."]
...
def test_prompt_block_is_empty_until_state_exists():
    ...
    # lightweight state block: only counts + failed paths; detailed failure patterns /
    ...
    assert "failure patterns" not in block.lower()
    assert "escalation hints" not in block.lower()
