...
async def test_unauth_classify():
    ...
    assert lead is True and "suspected unauthorized" in v
    # 200 but login page → not a lead
    v2, lead2 = recon_tools._classify_unauth(200, "please log in to continue", "text/html")
...
async def test_unauth_test_skips_destructive_and_flags_data(monkeypatch):
    ...
    assert "destructive" in res.lower() or "skipped" in res.lower()  # delete endpoint skipped
...
async def test_js_recon_auto_probes_endpoints(monkeypatch):
    ...
    assert "Unauthorized-access probe" in res or "unauthorized" in res.lower()
